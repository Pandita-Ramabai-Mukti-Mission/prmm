...
**One family sitewide: Geist Sans** (`--font-geist-sans`, loaded via `next/font` in `layout.tsx`) — headings and body both. `font-serif` was removed sitewide 2026-09-09 (`dev-backlog.md` #60): it had never been paired with an actual loaded serif webfont, so every heading was silently falling back to the browser's generic system serif rather than a deliberate typeface. **Don't reintroduce `font-serif`** (or any other Tailwind font utility) without loading a real font via `next/font/google` first and registering it in `globals.css`'s `@theme` block the way Geist already is — an unstyled fallback is worse than no heading-style rule at all.
