...
export default async function ProgramsIndex({
  ...
  return (
    <main id="main-content" className="flex flex-1 flex-col">
      {/* Page used to run all-white from header to footer — the flattest
          page on the site next to every other rebuilt page's bold-banded
          rhythm (site design audit, 2026-09-11). Split into a plain intro
          section and a tan card-grid band below, matching the same
          treatment ShapingThemeTemplate already uses for its own program
          grid. */}
      <section className="mx-auto w-full max-w-6xl px-6 pt-14 sm:px-12">
      ...
      <Reveal>
      <section className="mt-14 bg-[#f3efe7] px-6 py-14 sm:px-12">
          ...
          <div className="flex flex-wrap gap-2.5">
            <Link
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold ${
                !category ? "bg-coral text-white" : "border border-black/15 bg-white"
              }`}
            >
            ...
            {CATEGORIES.map((c) => (
              <Link
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold ${
                  category === c ? "bg-coral text-white" : "border border-black/15 bg-white"
                }`}
              >
                {c}
              </Link>
